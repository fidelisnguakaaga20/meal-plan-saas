...
export async function GET() {
  try {
    ...
    // If we don't have an active sub in DB, try Stripe to backfill (best-effort)
    ...
    // ✅ Always return 200 to avoid noisy console errors on the client.
    // If no active sub, return an explicit inactive payload.
    if (!profile?.subscriptionActive || !profile?.subscriptionTier || !profile?.stripeSubscriptionId) {
    ...
    // Still return 200 with inactive state to keep UI smooth
    return NextResponse.json({
      subscription: {
        stripeSubscriptionId: null,
        subscriptionActive: false,
